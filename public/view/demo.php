...
$stato=1;
$sql = "SELECT password, username, dominio FROM utenti WHERE username = :username and stato = :stato";
$stmt = $conn->prepare($sql);
$stmt->bindParam(':username', $username, PDO::PARAM_STR); // :username è il placeholder per il parametro
$stmt->bindParam(':stato', $stato, PDO::PARAM_STR); // :username è il placeholder per il parametro
$stmt->execute();
...
<head>
    ...
    <title>Demo Dashboard Kloe A.I.</title>
...
<div class="modal" id="settingsModal" tabindex="-1" role="dialog" aria-labelledby="settingsModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
                ...
                <form id="settingsForm" enctype="multipart/form-data" onsubmit="submitForm(event); return false;">
                   ...
                   <!-- <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="aptGPT" name="aptGPT" value="<?php echo $setting['apigpt']; ?>" readonly>
                    </div>

                                            <!-- Assistant GPT Setting -->
                    <div class="form-group">
                         <label for="assistantgpt">Assistant Name:</label>
                         <input type="text" class="form-control" id="assistantgpt" name="assistantgpt" value="<?php echo $setting['assistantgpt']; ?>" readonly>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="custom-control custom-switch">
                            <?php if (!!$setting['apigpt']) { ?>
                            <input type="checkbox" class="custom-control-input" id="chatGPTSwitch" name="chatGPTSwitch" checked="<?php echo $setting['onoffgpt']; ?>" readonly>
                            <?php } ?>
                            <?php if (!$setting['apigpt']) { ?>
                            <input type="checkbox" class="custom-control-input" disabled="true" readonly>
                            <?php } ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" class="form-control-file" id="logoxx" name="logoxx" required="true" readonly>
                    </div>

                    <!-- Color Picker for Header -->
                    <div class="form-group">
                        <label for="headerColor">Chat Color:</label>
                        <input type="color" class="form-control" id="headerColor" name="headerColor" readonly>
                    </div>

                    <!-- Submit Button -->
                    <button type="submit" class="btn btn-primary" id="alertButton">Save Settings</button>
                </form>
            </div>
        </div>
    </div>
        ...
        <aside class="left-sidebar" data-sidebarbg="skin6">
            ...
            <?php if ($verify_user['username']!='demo@demo.com') { ?>
...
<script>
  ...
  alertButton.addEventListener('click', function() {
    // Visualizza l'avviso con il testo desiderato
    alert('Only Premium Account!');
  });
</script>
        <script>
    $(document).ready(function() {
